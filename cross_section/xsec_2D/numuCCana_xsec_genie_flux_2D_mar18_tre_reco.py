...
inputdir = '/data/tmettler/data_flux/'
outputdir = inputdir+'xsec_2d_flux/' 
output_filedir = outputdir+'root_output/'
lib_function_dir = '/home/tmettler/numCC/helper_funcions/'
...
#!jupyter nbconvert --to script numuCCana_xsec_genie_flux_2D_mar18.ipynb
...
filename_overlay = 'NuCCana_overlay_V26_flux.root'
...
if 1:
    ...
    # read special file
    overlay_cut = ROOT.TChain(tree_name,"t_out")
    overlay_cut.Add( inputdir + 'NuCCana_overlay_V26_mar18_copy_out4.rootfriend.root')
...
if run:
    ...
    print 'Processing univers: true-reco matrix CV'
    end = time.time()
    ...
    for t_bin in range(len_theta):
        ...
        for m_bin in range(len_mom[t_bin]):
            ...
            for t2_bin in range(len_theta):
                ...
                overlay_cut.Draw('cos(TrackTheta):TrackMomMCS_mom'+'>>h_temp',weight_name+'*TunedCentralValue_Genie*(1'+this_cut+'&& numu_signal && track_end_uncontained)','',max_entries)
                overlay_cut.Draw('cos(TrackTheta):TrackMomRange_mu'+'>>h_temp_r',weight_name+'*TunedCentralValue_Genie*(1'+this_cut+'&& numu_signal && !track_end_uncontained)','',max_entries)
                h_temp.Add(h_temp_r)
...
if run:
    ...
    for para in range(num_para):
        for uni in range(num_universes):
            bin_counter_true = 0
            print 'Processing univers: ',uni,' true-reco matrix'
            end = time.time()
            print 'Time since start: ',(end - start)
            for t_bin in range(len_theta):
                print 'next theta',t_bin
                ...
                for m_bin in range(len_mom[t_bin]):
                    ...
                    for t2_bin in range(len_theta):
                        h_temp = ROOT.TH2F("h_temp",'h_temp',len_mom[t2_bin],array('f',mom_bins[t2_bin]),len_theta,array('f',theta_bins))
                        h_temp_r = ROOT.TH2F("h_temp_r",'h_temp_r',len_mom[t2_bin],array('f',mom_bins[t2_bin]),len_theta,array('f',theta_bins))
                        overlay_cut.Draw('cos(TrackTheta):TrackMomMCS_mom'+'>>h_temp',weight_name+'*'+weight_list[para]+'['+str(uni)+']*(1'+this_cut+'&& numu_signal && track_end_uncontained)','',max_entries)
                        overlay_cut.Draw('cos(TrackTheta):TrackMomRange_mu'+'>>h_temp_r',weight_name+'*'+weight_list[para]+'['+str(uni)+']*(1'+this_cut+'&& numu_signal && !track_end_uncontained)','',max_entries)
                        h_temp.Add(h_temp_r)
                        for m2_bin in range(len_mom[t2_bin]):
                            #print bin_counter_true,bin_counter_reco
